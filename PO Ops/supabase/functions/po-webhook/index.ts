...
function buildMetafields(
  ...
  const commentKey = poType === "mto" ? "_mto_comments" : "_replenishment_comments";
  const commentMetaKey = poType === "mto" ? "mto_comments" : "replenishment_comments";
  const comments = (sourceOrder.note_attributes ?? []).find((na: any) => na.name === commentKey)?.value ?? "";
  if (comments) {
    metafields.push({ namespace: "custom", key: commentMetaKey, value: comments, type: "multi_line_text_field" });
  }
...
async function createPoDraftOrder(
  ...
  const draftOrderBody: any = {
    draft_order: {
      ...
      metafields: buildMetafields(poType, sourceOrderId, sourceOrderName, token, sourceOrder)
    }
  };
...
Deno.serve(async (req: Request) => {
  ...
  for (const [poType, items] of Object.entries(groups)) {
    ...
    await writeToSheets({
      ...
      po_comments:     (order.note_attributes ?? []).find((na: any) => na.name === (poType === "mto" ? "_mto_comments" : "_replenishment_comments"))?.value ?? "",
