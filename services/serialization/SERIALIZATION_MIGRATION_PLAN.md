...
|--------------------|----------------------------|--------------------|-----|--------|---------------|-----|---------|
| `customer_order`   | `TM{FY}-{CODE}-{SEQ}`      | `TM27-KAHSR-00001` | 16  | store  | yes           | 5   | order-serial webhook (state_code set) |
| `customer_service` | `TS{FY}-{CODE}-{SEQ}`      | `TS27-KAHSR-00001` | 16  | store  | yes           | 5   | repair-complete (PAID) + CAD/design |
| `free_service`     | `FS{FY}-{CODE}-{SEQ}`     | `FS27-KAHSR-00001` | 16  | store  | yes           | 5   | repair-complete (FREE: `repair-free`/`free-repair`) |
| `b2b`              | `AURA-{CODE}-{SEQ}`       | `AURA-KAHSR-0001`  | 15  | store  | no            | 4   | `make-transfer` |
| `delivery_challan` | `DC-{CODE}-{SEQ}`         | `DC-KAHSR-0001`    | 13  | store  | no            | 4   | `make-challan` (cancel `cancel-challan`) |
| `po`               | `PO-{CODE}-{SEQ}`        | `PO-KAHSR-00001`   | 14  | store  | no            | 5   | HQ acknowledge (po-ops) |
| `exchange_note`    | `EXC{FY}-{CODE}-{SEQ}`   | `EXC27-KAHSR-0001` | 16  | store  | yes           | 4   | `/api/serial/allocate` (store code required) |
| `voucher`          | `VCH{FY}-{CODE}-{SEQ}`  | `VCH27-KAHSR-0001` | 16  | store  | yes           | 4   | `/api/serial/allocate` (store code required) |

`{CODE}` = compound store code rendered **without its hyphen** (`KA-HSR` → `KAHSR`); the full hyphenated
`KA-HSR` still drives the counter key and the staff-facing `custom.state_code` metafield.
`{FY}` = 2-digit FY-end (IST). `{SEQ}` = zero-padded.

Doc types removed/renamed: `memo`→`delivery_challan`, `transfer`→folded into `b2b`, `repair`→split into
`customer_service` (paid) + `free_service` (free). Legacy `credit_note` rows are wiped with everything else (prototype).

**EXC/VCH are now per-store** (was global): the `/api/serial/allocate` caller MUST pass `stateCode`, and
voids must use the full `serial_code` (seq is no longer unique across stores within an FY).
