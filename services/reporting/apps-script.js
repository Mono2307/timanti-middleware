 ...
 *   Credit Ledger  GET /api/recon-ledger      — voucher/exchange-note/advance/refund lifecycle
 *                                               (summary/outstanding/tieout/detail). Put `refund` in
 *                                               the Type filter to see refunds alone; that view has no
 *                                               fixed column list, so its refund fields (refunded_at,
 *                                               refund_mode, gateway_ref) appear on their own.
...
const REPORTS = {
  sales: {
    ...
    cols: ['stage','payment_type','draft_name','order_name','day','customer',
           // Who raised the sale (custom.employee_name).
           'sales_staff',
           'place_of_supply','shipping_state',
           // category is the Shopify product type; option_1..3 are the variant option values split
           // out of variant_title so they can be filtered and pivoted.
           'product_title','category','variant_title','option_1','option_2','option_3',
           'sku','hsn','qty','gross_sales','discount','net_sales',
           ...
           'taxable_value','igst','sgst','cgst','custom_serial',
           // Money movement leg by leg, spread rightward — i* is money in, r* money out. Replaces the
           // old advance/final two-slot framing. r*_ref is the gateway UTR. No per-leg date or type:
           // `day` is the date this report is keyed on, and the mode carries the tender.
           'i1_value','i1_mode',
           'i2_value','i2_mode',
           'i3_value','i3_mode',
           'i4_value','i4_mode',
           ...
           // Totals, to the right of the legs they are computed from. amount_paid is GROSS collected
           // and amount_refunded what went back; net_collected is the figure to sum for a collections
           // total. All three ride along so a refund can be reconciled against the legs.
  ...
  adjustments: {
    ...
    cols: ['name','created_at','customer','place_of_supply','shipping_state','custom_serial','hsn',
           ...
           // refund_value is money OUT and is deliberately NOT part of any sales sum — it sits in its
           // own column so the existing totals keep meaning exactly what they meant before.
           // document_state is only populated on refund rows whose draft never became an order.
           'exchange_note_value','old_gold_value','advance','amount_to_be_collected','amount_paid','total_price',
           'refund_value','document_state','instruments_issued','instruments_redeemed'],
  },
  recon: {
...
function generate_(key) {
  ...
  // Clear the width the tab ACTUALLY has, not the width we are about to write.
  //
  // This used to clear max(cols.length, 30). That is fine while a report only ever grows, but the
  // moment a schema SHRINKS the old columns past the new width are never cleared — so the stale tail
  // of the previous header sits there looking like duplicate columns, with stale data under it. The
  // sales report going 52 → 46 → 42 columns left ten dead columns doing exactly that
  // (…installments, amount_pending, net_to_collect, payment_mode, installments…).
  //
  // getLastColumn() is the real extent of the tab, so this clears whatever the last run left behind
  // however wide it was.
  const clearWidth = Math.max(cols.length, sh.getLastColumn(), 30);
  if (lastRow >= startRow) sh.getRange(startRow, 1, lastRow - startRow + 1, clearWidth).clearContent();
